...
type QiaoMcConfluxProviderProps = IInpageProviderConfig & {
  timeout?: number;
};
...
class ProviderConflux extends ProviderConfluxBase implements IProviderConflux {
  ...
  constructor(props: QiaoMcConfluxProviderProps) {
    super({
      ...props,
  ...
  private _handleChainChanged({ chainId, networkId }: Network = {}) {
    if (
      ...
      this._log.error('Qiaomc: Received invalid network parameters. Please report this bug.', {
        chainId,
        networkId,
      });
      return;
