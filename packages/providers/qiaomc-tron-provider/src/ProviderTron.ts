...
type QiaoMcTronProviderProps = IInpageProviderConfig & {
  timeout?: number;
};
...
export const TRON_REQUEST_ACCOUNTS_LOCAL_KEY = 'qiaomc_tron_request_accounts_local_key';
...
class QiaoMcTronWeb extends TronWeb {
...
class ProviderTron extends ProviderTronBase implements IProviderTron {
  ...
  constructor(props: QiaoMcTronProviderProps) {
    super({
      ...props,
  ...
  private _registerTronWeb(nodes: Nodes): { tronWeb: TronWeb; sunWeb: SunWeb } | null {
    ...
    const tronWeb: TronWeb = new QiaoMcTronWeb(
      {
        ...nodes,
      },
      this,
    ...
    const tronWeb1: TronWeb = new QiaoMcTronWeb(
      {
        ...nodes,
      },
      this,
    ...
    const tronWeb2: TronWeb = new QiaoMcTronWeb(
      {
        ...nodes,
      },
      this,
  ...
  private async _initialize() {
    try {
      ...
      if (window.tronWeb !== undefined) {
        this._log.warn(
          'QiaoMc: TronWeb is already initiated. Qiaomc will overwrite the current instance',
        );
      }
      ...
      Object.defineProperty(tronWeb, 'defaultAddress', {
        get() {
          if (!self._connected) {
            self._log.warn(
              'QiaoMc: We recommend that DApp developers use $qiaomc.tron.request({method: "tron_requestAccounts"}) to request users’ account information at the earliest time possible in order to get a complete TronWeb injection.',
            );
  ...
  private _handleAccountsChanged(accounts: string[]) {
    ...
    if (!Array.isArray(accounts)) {
      this._log.error(
        'Qiaomc: Received invalid accounts parameter. Please report this bug.',
        accounts,
      );
      _accounts = [];
    }
    ...
    for (const account of _accounts) {
      if (typeof account !== 'string') {
        this._log.error('Qiaomc: Received non-string account. Please report this bug.', accounts);
        _accounts = [];
        break;
      }
    }
