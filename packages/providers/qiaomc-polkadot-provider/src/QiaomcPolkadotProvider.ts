...
import QiaoMcInjected from './inject/Injected';
...
export type QiaoMcPolkadotProviderProps = IInpageProviderConfig & {
  timeout?: number;
};
...
class ProviderPolkadot extends ProviderPolkadotBase implements IProviderPolkadot {
  ...
  constructor(props: QiaoMcPolkadotProviderProps) {
    super({
      ...props,
  ...
  private createMessage(payload: IMessagePayload): IPostMessage {
    return {
      id: 2,
      origin: 'QiaoMc Polkadot Provider',
      ...payload,
    };
  }
...
const registerPolkadot = (provider: ProviderPolkadot, name = 'QiaoMc', version = '1.0.0') => {
  try {
    const enableFn = async (originName: string): Promise<Injected> => {
      await provider.web3Enable(originName);
      return new QiaoMcInjected(provider);
    };

    injectExtension(enableFn, { name: name ?? 'QiaoMc', version: version ?? '1.0.0' });
  } catch (error) {
    console.error(error);
  }
};
