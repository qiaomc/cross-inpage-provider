...
import { hexToBytes } from '@qiaomcfe/cross-inpage-provider-core';

import { ALL_PERMISSION_TYPES } from './types';

import type { ProviderSui } from './QiaomcSuiProvider';
...
class QiaomcSuiStandardWallet implements Wallet {
  readonly version = '1.0.0' as const;
  readonly _name = 'QiaoMc Wallet' as const;
...
export function registerSuiWallet(provider: ProviderSui, options?: WalletInfo) {
  try {
    registerWallet(new QiaomcSuiStandardWallet(provider, options));
  } catch (error) {
    console.error(error);
  }
}
