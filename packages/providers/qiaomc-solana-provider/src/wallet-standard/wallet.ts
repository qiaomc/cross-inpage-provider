...
import {QiaoMcSolanaWalletAccount} from './account';
...
export const QiaoMcNamespace = 'qiaomc:';

export type QiaoMcFeature = {
    [QiaoMcNamespace]: {
        qiaomc: ProviderSolana;
    };
};


export class QiaoMcSolanaStandardWallet implements Wallet {
  readonly #listeners: { [E in StandardEventsNames]?: StandardEventsListeners[E][] } = {};
  readonly #version = '1.0.0' as const;
  readonly #name = 'QiaoMc' as const;
  readonly #icon: WalletIcon;
  #account: QiaoMcSolanaWalletAccount | null = null;
  ...
  constructor(provider: ProviderSolana, options: WalletInfo) {
      if (new.target === QiaoMcSolanaStandardWallet) {
          Object.freeze(this);
      }
  ...
  #connected = () => {
      ...
      if (address) {
          ...
          if (!account || account.address !== address || !bytesEqual(account.publicKey, publicKey)) {
              this.#account = new QiaoMcSolanaWalletAccount({ address, publicKey });
