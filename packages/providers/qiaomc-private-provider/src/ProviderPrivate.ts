...
import { IInpageProviderConfig, ProviderBase, switchDefaultWalletNotification, switchNetworkNotification } from '@qiaomcfe/cross-inpage-provider-core';
import { consts } from '@qiaomcfe/cross-inpage-provider-core';

export interface IQiaoMcWalletInfo {
...
class ProviderPrivate extends ProviderBase {
        ...
        if (method === METHODS.wallet_events_ext_switch_changed) {
          ...
          if (!walletInfoLocal || (walletInfoLocal && walletInfoLocal.platformEnv.isExtension)) {
            try {
              ...
              this.notifyDefaultWalletChanged(params as IQiaoMcWalletInfo)
            } catch (e) {
              console.error(e);
            }
          }
  ...
  notifyDefaultWalletChanged(params: IQiaoMcWalletInfo) {
