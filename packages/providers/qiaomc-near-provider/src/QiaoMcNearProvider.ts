...
import { IInpageProviderConfig } from '@qiaomcfe/cross-inpage-provider-core';
...
export type QiaoMcNearWalletProps = {
...
export type QiaoMcWalletAccountProps = {
  wallet: QiaoMcNearProvider;
  connection: unknown;
  accountId: string;
};
...
type QiaoMcNearProviderEventsMap = {
...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
declare interface QiaoMcNearProvider {
  // TODO add request type define
  on<U extends keyof QiaoMcNearProviderEventsMap>(
    event: U,
    listener: QiaoMcNearProviderEventsMap[U],
    context?: any,
  ): this;
  emit<U extends keyof QiaoMcNearProviderEventsMap>(
    event: U,
    ...args: Parameters<QiaoMcNearProviderEventsMap[U]>
  ): boolean;
}
...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
class QiaoMcNearProvider extends ProviderNearBase {
  _enablePageReload?: boolean = false;
  _connectEagerly?: boolean = true;
  _authData: NearAccountInfo = DEFAULT_AUTH_DATA;
  _authDataKey = '@QiaoMcNearWalletAuthData';
  _account?: QiaoMcWalletAccount | null;
  ...
  constructor({
    ...
    if (!networkId) {
      // throw new Error('QiaoMcNearWallet init error: networkId required.');
    }
  ...
  account() {
    ...
    if (!this._account || this._account.accountId !== accountId) {
      this._account = new QiaoMcWalletAccount({
        wallet: this,
...
class QiaoMcWalletAccount extends Account {
  _wallet: QiaoMcNearProvider;

  constructor({ wallet, connection, accountId }: QiaoMcWalletAccountProps) {
...
export { QiaoMcNearProvider, QiaoMcWalletAccount, serializeTransaction };
